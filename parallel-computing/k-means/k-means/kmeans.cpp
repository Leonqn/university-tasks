...
//	#pragma omp parallel for reduction(+:distance_sqr)
...
size_t FindNearestCentroid(const Points& centroids, const Point& point) {
	...
	//#pragma omp parallel for reduction(min: min_distance, max: centroid_index)
...
int main(int argc , char** argv) {
	...
	output.close();
	std::cout << omp_get_wtime() - start << " seconds\n";
	return 0;
}